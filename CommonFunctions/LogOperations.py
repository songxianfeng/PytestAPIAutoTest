...
class LogOperations(object):
    ...
    def __init__(self):
        ...
        #Jenkins目录
        handler=logging.FileHandler(os.getcwd()+"/Logs/ApiTestLog"+time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) +".log")
        
        #本地执行
        #handler=logging.FileHandler("../../Logs/ApiTestLog"+time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) +".log")
